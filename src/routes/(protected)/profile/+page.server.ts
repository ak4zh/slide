	...
	default: async (event) => {
		...
		try {
			...
			if (user.email !== form.data.email) {
				//TODO: get emailaddress to change for orm not just in attributes.  setUser not working... weird
				...
				await db.update(table.keys).set({
					id: 'emailpassword:' + form.data.email
				}).where(eq(table.users.id, 'emailpassword:' + user.email))
				auth.updateUserAttributes(user.userId, {
					verified: false
				});
				//await auth.invalidateAllUserSessions(user.userId);
